...
function FieldSet({ children, editItem }: any) {
    ...
    return (
        <fieldset>
            {children.map((item: any, i: number) => React.createElement(blocks[item.type].component,{ key: i, ...item, editItem: (key: string, value: any) => editChildrenItem(item, key, value) }))}
        </fieldset>
    )
}
